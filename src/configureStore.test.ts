import { configureStore, Middleware } from "./configureStore";
...
describe("configureStore", () => {
  ...
  describe("middlewares", () => {
    it("called when store dispatched", () => {
      const state = { name: "Bob" };
      const action1 = { type: "xxx" };
      const spy = jest.fn();
      const middleware: Middleware<any, any> = (store) => (next) => (
        action
      ) => {
        spy();
        return next(action);
      };
      const store = configureStore(() => state, state, [middleware]);
      store.dispatch(action1);
